...
6. **Show the schematic at milestones** — Call `draw_schematic` after initial design and after significant modifications so the user can see the circuit. When the response includes a `schematic_url` field, always present it to the user as a clickable link so they can view the schematic image directly in their browser — for example: "Here's the schematic: [View schematic](https://mcp.clanker-lover.work/schematics/abc123.png)". The same applies to `auto_design` responses that include `schematic_url`.
